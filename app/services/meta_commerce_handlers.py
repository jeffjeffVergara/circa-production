    ...
    if btn == "ACEPTO":
        try:
            ...
            if bodega_ac:
                ...
                # Tras T&C siempre pedir crear clave (Express ya no activa sin PIN)
                ses = db.get_session(ctx.telefono)
                fase = (ses or {}).get("fase") or ""
                if fase == "express_tyc":
                    try:
                        db.update_bodega(bod_id, {"onboarding_fase": "express_pin"})
                    except Exception:
                        pass
                ...
                logger.info(
                    "Contract signed for bodega %s (fase=%s), hash=%s — pidiendo PIN",
                    bod_id,
                    fase,
                    contract_hash,
                )
