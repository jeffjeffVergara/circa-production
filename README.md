...
Backend **MVP** de **Circa**: crédito embebido orientado a **bodegas en Perú**. El dueño de la bodega opera casi todo por **WhatsApp** (y flujos nativos de Meta), con **FastAPI**, datos en **Supabase** y mensajes vía **Meta Cloud API** (persiste integración **Twilio** como legado).

## Arquitectura principal

| Pieza | Rol |
| --- | --- |
| `app/main.py` | App FastAPI: webhooks Twilio/Meta, flows cifrados, APIs REST, páginas estáticas (`/catalogo`, `/pin`), salud y textos legales. |
| `app/state_machine.py` | Máquina de estados del chat (`handle_message`): sesión en BD, fases (bienvenida, RUC/DNI, menú, carrito, etc.) y señales a plantillas o Meta. |
...
Variables en `.env` (ver `.env.example`). Referencia en `app/config.py`: Supabase, Twilio, Meta, IDs de flows, APIs Perú, etc.
...
Para exponer el webhook (desarrollo): túnel tipo `ngrok http 8000` y configurar la URL en Meta/Twilio según corresponda.
